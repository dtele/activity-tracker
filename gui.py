...
def for_filename_read_popup():
    ...
    global filename_read
    if exists(rf'{getcwd()}/logs'):
        filename_read = filedialog.askopenfilename(initialdir=rf'{getcwd()}/logs')
    else:
        print('No logs folder detected. Press start logging to make one.')
